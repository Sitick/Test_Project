...
// 2. Защита от дребезка не реализована программно поскольку есть RC фильтр, а также довольно высокая частота опроса
// 3. Программное обеспечение будет использоваться для панелей управления различных устройств (то есть это не обычная клавиатура компьюетера)
